...
type Workout = {
  presetId?: string; // 開始時に選択したメニュー ID
  id: string;
...
- バックアップ:
  ...
  - 復元確認・データ選択ダイアログでクラウド復元を選ぶと、完了まで見出しを「クラウドから復元中…」に変更し、通信待ちの案内を表示します。復元中は確認文と両方の選択ボタンを非表示にして、操作できないことを明確にします。失敗時は確認文とボタンを戻して再操作できる状態にします。
...
- **実施日の色**: メニューから開始した記録は `Workout.presetId` を保存し、週・月カレンダーの実施日をそのメニューの現在の設定色で表示する。当日・選択日も色を表示し、選択日は外枠で区別する。複数メニューの記録がある日は記録順で最初のメニューの色を使う。旧記録・メニュー未選択・削除済みメニューは赤で表示する。
...
- **種目一覧**: `selectedWorkouts` をカード表示。各カードはカード全体を覆う詳細ボタンでタップ / Enter / Space に対応し、削除ボタンとは兄弟要素に分ける。
  - カードヘッダに部位名と種目名、削除ボタンを表示する。部位名は検索結果と同じ12pxのグレー文字で種目名の先頭に表示し、右に「｜」を挟んで種目名と上下中央に揃える。ヘッダ左のライン色は部位の表示色（`partColors`）を反映する。
...
- **通常モード**:
  - 種目名の検索欄から全部位の種目を部分一致で検索できる。前後の空白、英字の大文字・小文字、全角・半角の違いを吸収する。検索欄のプレースホルダーは「種目名で検索」。検索中は種目名の先頭に部位名を置き、各項目の左ボーダーで部位カラーを表した検索結果と件数（0件の場合は該当なしの案内）を表示し、タップで種目を追加する。「クリア」で元の部位別一覧に戻る。
...
- 「メニューの色」で8色から選択でき、名称・種目・スケジュールと同時に保存する。保存前に戻ると色の変更も破棄する。色の変更は過去の実施日にも反映する。旧データ・不正な色は既定の赤に正規化する。
- 編集画面に削除ボタンは置かず、削除はトレーニングメニュー一覧から行う。
- 編集: 名称変更（保存時に空なら「名称未設定」）、種目の削除・ドラッグハンドルによる並び替え。ドラッグ中は行が浮き上がった表示になり、指またはカーソルへ追従する。
- メニュー名の下に「種目の選択」「スケジュール」「メニューの色」の順で、独立したカードとして余白を空けて表示する。
...
- **外観**: ダーク / ライトの切り替えスイッチを表示する。切り替えた外観は `state.themeMode` に保存され、アプリ全体の配色に反映される。
  - ブラウザの `theme-color` はヘッダー色（`--panel`）に追従する。スマホ幅およびPWAのスタンドアロン表示ではページ背景も同じ色に揃え、画面遷移時のステータスバー背景が黒くならないようにする。
